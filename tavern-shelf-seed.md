...
相同卡片数据，或名称和作者都非空且一致时，自动建立展示关联；数据指纹覆盖完整 JSON，忽略排版和对象键顺序，保留未知扩展字段。关联卡在排序后相邻排列，仍各自展示；搜索、筛选和数量统计按单张卡处理。作者为空且数据变化时，不猜测关联。

详情显示当前卡自己的封面，用轻量的关联卡缩略图切换到其他独立卡。低频的建立关联、移出关联操作放在更多菜单中。不存在主卡、默认修订或跨卡选择书架封面。作者版本号仅作展示，收录时间只表达这张卡何时收藏。
