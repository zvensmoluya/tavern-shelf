...
手动收藏也支持粘贴附件下载链接或已适配社区的角色卡地址；界面只提供一个「链接导入」入口，下载后复用同一套 Core 收录流程，不引入社区浏览或登录系统。
...
Shelf 的收录判断与 Player 的可播放性校验分离。只要能识别角色卡身份，就保存原始 Source；JSON 无封面、名称缺失、字段不规范或部分损坏均不构成拒收理由。可读取字段用于展示，无法展示的部分记录读取提示，原始字节保持不变。无法识别的普通文件留在原处。文件大小和解压资源限制属于资源保护，不是角色卡质量判断。
...
每份不同原件是一张独立收藏卡，拥有自己的封面、收藏时间、收藏信息和下载入口。完全相同文件按 SHA-256 去重，不因文件名不同而新增收藏。

相同卡片数据，或名称和作者都非空且一致时，自动建立展示关联；数据指纹覆盖完整 JSON，忽略排版和对象键顺序，保留未知扩展字段。关联卡默认在书架使用最多三张真实封面直立错层叠放，叠卡与普通卡共用同尺寸封面区域且标题对齐，以当前排序下首张匹配卡作为正面，可切换为逐张展开；搜索、筛选和数量统计按单张卡处理，部分匹配时显示匹配数与总数。作者为空且数据变化时，不猜测关联。

详情显示当前卡自己的封面，用标题附近常驻的真实封面缩略图切换到其他独立卡，标记选中项并列出文件名与作者版本，下方显示收藏时间和差异摘要。缩略图优先展示匹配项；本次浏览期间再次打开一叠时恢复上次查看且仍符合筛选的原件。低频的建立关联、移出关联操作放在更多菜单中。不存在主卡、默认修订或跨卡选择书架封面。作者版本号仅作展示，收录时间只表达这张卡何时收藏。
