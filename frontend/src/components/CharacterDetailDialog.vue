...
import type { Character, CharacterBookEntry, RegexScript } from "@/types";
...
function entryFlags(entry: CharacterBookEntry): string[] {
  const flags: string[] = [];
  if (entry.constant) flags.push("常驻");
  if (entry.selective) flags.push("选择性匹配");
  if (entry.useRegex) flags.push("Regex keys");
  if (entry.caseSensitive) flags.push("区分大小写");
  return flags;
}
      ...
      >
        ...
        <div data-testid="detail-scroll" class="shelf-scrollbar min-h-0 min-w-0 overflow-x-hidden overflow-y-auto overscroll-contain max-[610px]:h-[52vh]">
          <article class="w-full max-w-[760px] px-11 pb-14 pt-14 max-[860px]:px-7 max-[860px]:pb-10 max-[860px]:pt-12 max-[610px]:px-5 max-[610px]:pt-7">
            ...
            <ExpandableText v-if="profile.description" :text="profile.description" variant="lead" label="展开完整角色描述" class="mb-6" />
            ...
            <ContentSection v-if="greetings.firstMessage || greetings.alternate.length || greetings.groupOnly.length" title="开场" :meta="`${greetings.totalCount} 个`">
              <ExpandableText v-if="greetings.firstMessage" :text="greetings.firstMessage" :limit="390" variant="opening" label="展开完整开场" />
              <div v-if="greetings.alternate.length" class="mt-3 grid gap-2">
                <ShelfDisclosure v-for="(message, index) in greetings.alternate" :key="`alt-${index}`" :title="`Alternate Greeting ${index + 1}`" compact>
                  <p class="whitespace-pre-wrap">{{ message }}</p>
                </ShelfDisclosure>
              </div>
              <div v-if="greetings.groupOnly.length" class="mt-3 grid gap-2">
                <ShelfDisclosure v-for="(message, index) in greetings.groupOnly" :key="`group-${index}`" :title="`群聊开场 ${index + 1}`" compact>
                  <p class="whitespace-pre-wrap">{{ message }}</p>
                </ShelfDisclosure>
              </div>
            </ContentSection>
            ...
            <ContentSection
              v-if="manifest.extensions.length || manifest.assets.length || manifest.interaction.hasHtml || manifest.interaction.hasJavaScript || manifest.interaction.hasInteractiveExtension"
              title="扩展与交互内容"
            >
            ...
            <ContentSection v-if="profile.creatorNotes || Object.keys(profile.creatorNotesMultilingual || {}).length" title="创作者说明">
              ...
              <div v-if="Object.keys(profile.creatorNotesMultilingual || {}).length" class="mt-3 grid gap-2">
                <ShelfDisclosure v-for="(text, language) in profile.creatorNotesMultilingual" :key="language" :title="`${String(language).toUpperCase()} 创作者说明`" compact>
